pipeline {
  parameters {
    ...
    choice(name: 'TARGET', choices: ['all', 'ubuntu-18.04', 'ubuntu-20.04', 'ubuntu-22.04', 'rhel-8', 'rhel-9'], description: 'Select Target Distro')
  }
  agent {
    label 'smd_ibm'
  }
    ...
    stage('Build packages') {
      ...
      steps {
        withCredentials([string(credentialsId: 'github_access_token_zabbix_s390x_deploy', variable: 'GITHUB_TOKEN')]) {
          script{
            def target = ""
            if(params.TARGET != "all") { target = params.TARGET }
